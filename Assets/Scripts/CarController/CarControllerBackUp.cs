...
public class CarControllerBackUp : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        frontWheelR.localEulerAngles = new Vector3(frontWheelR.localEulerAngles.x + carSphere.velocity.magnitude * 10, (Input.GetAxis("Horizontal") * 15), 0);
        frontWheelL.localEulerAngles = new Vector3(frontWheelL.localEulerAngles.x + carSphere.velocity.magnitude * 10, (Input.GetAxis("Horizontal") * 15), 0);
    ...
    public void DriftCheck()
    {
        ...
        if (drifting)
        {
            ...
            if (driftDirection == 1)
            {
                tempControl = ExtensionMethods.Remap(Input.GetAxis("Horizontal"), -1, 1, -1, 2);
                anim.SetBool("DriftR", true);
            }
            else
            {
                anim.SetBool("DriftL", true);
                tempControl = ExtensionMethods.Remap(Input.GetAxis("Horizontal"), -1, 1, 2, -1);
            }

            float tempPowerControl;
            if (driftDirection == 1)
            {
                tempPowerControl = ExtensionMethods.Remap(Input.GetAxis("Horizontal"), -1, 1, .2f, 1);
            }
            else
            {
                tempPowerControl = ExtensionMethods.Remap(Input.GetAxis("Horizontal"), -1, 1, 1, .2f);
            }
            Brake();
        ...
        {
            if (Input.GetAxisRaw("LeftTrigger") == 0) brakeLights.enabled = false;
    ...
    public void BoostCheck()
    {
        if (boosting)
        {
            if (Input.GetButtonUp("Boost") || driftPoints <= 0 || boostReset)
            {
                boosting = false;
                if (boostPlaying)
                {
